...
def freshdesk_source(
    ...
    initial = _freshdesk_iso(since) if since else "2022-01-01T00:00:00Z"
    end = _freshdesk_iso(until) if until else None
...
def _freshdesk_iso(value: str) -> str:
    """Normalize run-slice date to ISO8601 for Freshdesk updated_since."""
    if "T" in value:
        return value
