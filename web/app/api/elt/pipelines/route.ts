...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (parsed.data.dbtProjectId) {
      try {
        await linkDbtProjectToPipeline(auth.user.id, parsed.data.dbtProjectId, result.pipeline.id);
        const refreshed = await db.eltPipeline.findUnique({ where: { id: result.pipeline.id } });
        return NextResponse.json({ pipeline: refreshed ?? result.pipeline }, { status: 201 });
      } catch (e) {
        const msg = e instanceof Error ? e.message : "Pipeline created but dbt link failed";
        return NextResponse.json({ pipeline: result.pipeline, warning: msg }, { status: 201 });
      }
    }
