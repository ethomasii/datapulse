    ...
    <div className="w-full min-w-0 max-w-5xl space-y-8">
      ...
      <div>
        <div className="inline-flex items-center gap-2 text-teal-600 dark:text-teal-400">
          ...
          <span className="text-sm font-semibold uppercase tracking-wide">Data Partitioning</span>
        </div>
        <h1 className="mt-2 text-2xl font-bold text-slate-900 dark:text-white">Partitions & Backfills</h1>
        <p className="mt-3 text-slate-600 dark:text-slate-300">
          Configure how each pipeline slices its data — by date, key, region, or customer — then run targeted
          backfills over any range of partition values. Each partition launches an independent run you can
          monitor in{' '}
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-dashed border-slate-200 p-10 text-center dark:border-slate-700">
          ...
          <p className="mt-1 text-sm text-slate-500">Create a pipeline first, then configure partitions here.</p>
          <Link
            href="/builder"
      ...
      <section className="rounded-2xl border border-dashed border-slate-200 bg-slate-50/80 p-6 dark:border-slate-700 dark:bg-slate-900/40">
        ...
        <div className="mt-3 grid grid-cols-1 gap-4 text-sm text-slate-600 dark:text-slate-300 sm:grid-cols-3">
          <div>
            <strong className="font-medium text-slate-800 dark:text-slate-200">Date partition</strong>
          ...
          <div>
            <strong className="font-medium text-slate-800 dark:text-slate-200">Key partition</strong>
            <p className="mt-1">Slice by a discrete value — customer ID, region, tenant. Each partition value becomes an isolated run.</p>
          </div>
          <div>
            <strong className="font-medium text-slate-800 dark:text-slate-200">Backfill</strong>
            <p className="mt-1">Re-run one or more partition values on demand. Each partition launches its own run record you can track and retry independently.</p>
          </div>
        </div>
      </section>
    </div>
...
function PartitionEditor({
  ...
  async function saveConfig() {
    ...
    try {
      ...
      if (!res.ok) throw new Error('Failed to save partition config');
      setSavedFlash(true);
  ...
  return (
    <div className="space-y-6">
      {/* Config form */}
      <div>
        <h3 className="text-sm font-semibold text-slate-900 dark:text-white mb-3">Partition configuration</h3>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div>
            <label className="block text-xs font-medium text-slate-500 mb-1">Partition type</label>
            <select
              value={type}
          ...
          {type !== 'none' && (
            <div>
              <label className="block text-xs font-medium text-slate-500 mb-1">
                {isDate ? 'Date/timestamp column' : 'Partition key column'}
              </label>
      ...
      {type !== 'none' && column.trim() && (
        <div className="rounded-xl border border-teal-200 bg-teal-50/40 p-5 dark:border-teal-800 dark:bg-teal-900/10">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mb-4">
            Select a range of partition values, preview, then launch. Each value creates an independent run
          ...
          {previewReady && (
            <div className="mt-4">
              {backfillRows.length === 0 ? (
                <p className="text-sm text-slate-500">No partition values generated. Check your date range or values.</p>
              ) : (
                <>
