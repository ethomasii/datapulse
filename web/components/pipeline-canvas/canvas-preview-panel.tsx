...
function PreviewPane({ title, table, pipelineId, config, className, onDiagnosticChange }: PreviewPaneProps) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/elt/pipelines/${encodeURIComponent(pipelineId)}/preview`, {
        ...
        body: JSON.stringify({ table, config, limit: 8, includeProfiles: true }),
      });
  ...
  function renderPreviewTable(data: PreviewResult) {
    const rows = data.rows ?? [];
    const columnNames =
      data.columns?.length ? data.columns : rows[0] ? Object.keys(rows[0]) : [];
    if (!columnNames.length) return null;

    const profiles = enrichProfilesFromSampleRows(data.columnProfiles ?? {}, columnNames, rows);
    ...
    return (
      <table className="w-full text-left text-[10px]">
        <thead className="sticky top-0 bg-slate-100 dark:bg-slate-900">
          <tr>
            {columnNames.map((c) => (
              <th key={c} className="min-w-[5.5rem] px-2 py-1 font-semibold text-slate-600 dark:text-slate-300">
                {c}
              </th>
            ))}
          </tr>
            ...
            <tr className="border-t border-slate-200/80 dark:border-slate-700/80">
              {columnNames.map((c) => (
                <th key={`${c}-profile`} className="min-w-[5.5rem] px-2 pb-1.5 align-top font-normal">
                  <ColumnProfileBar profile={profiles[c]} />
                </th>
              ))}
            </tr>
          ) : null}
  ...
  return (
    <div className={clsx("flex min-h-0 min-w-0 flex-1 flex-col border-r border-slate-200 last:border-r-0 dark:border-slate-800", className)}>
      ...
      <div className="min-h-[3.5rem] flex-1 overflow-auto p-2">
        ...
        ) : result && (result.rows?.length || result.columns?.length) ? (
