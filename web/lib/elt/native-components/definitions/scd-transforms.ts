...
export const scdType1Component: NativeComponentDefinition = {
  id: "scd_type_1",
  ...
  compile(config) {
    ...
    const updatePy = updateCols.length
      ? `[${updateCols.map((c) => JSON.stringify(c)).join(", ")}]`
      : `[c for c in _staging.columns if c not in ${keysPy}]`;
    const python = [
...
export const scdType2Component: NativeComponentDefinition = {
  id: "scd_type_2",
  ...
  compile(config) {
    ...
    const { outSchema, outName } = outputParts(output);
    const keysPy = `[${keys.map((k) => JSON.stringify(k)).join(", ")}]`;
    const comparePy = compareCols.length
      ? `[${compareCols.map((c) => JSON.stringify(c)).join(", ")}]`
      : `[c for c in _staging.columns if c not in ${keysPy} + [${JSON.stringify(effFrom)}, ${JSON.stringify(effTo)}, ${JSON.stringify(isCurrent)}]]`;
    const python = [
