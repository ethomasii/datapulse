...
export const TOP_COMPONENT_ROUTES: Record<string, ComponentRoute> = {
  ...
  mcp_tool_call: { target: "python", badge: "native", hint: "Deterministic MCP tool ingest — workspace server registry" },
  litellm_agent: { target: "python", badge: "native", hint: "LLM agent with MCP tools (LiteLLM loop)" },
  openai_agent: { target: "python", badge: "native", hint: "OpenAI agent — routes to litellm_agent native" },
...
const CATEGORY_DEFAULTS: Record<string, ComponentRoute> = {
  ...
  ai: { target: "python", badge: "native", hint: "Native LLM / MCP enrichment (litellm_agent, inference, evaluator)" },
